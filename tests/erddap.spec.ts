...
import { fetchSamples, fetchTemperatures, fetchCoordinates, fetchInfo, fetchSpecies, fetchStations } from '@/utils/erddap/db';
...
test('fetchSamples', async () => {
    ...
    await expect(Object.keys(firstSample).sort()).toMatchObject(["abun", "animal", "species", "station", "title", "year"].sort());
    ...
    const samplesWithStationParam = await fetchSamples([{ key: "Station", comparator: "=", value: "Fox Island" }]);
...
test("fetchTemperatures", async () => {
    ...
    await expect(Object.keys(temperatures[0]).sort()).toMatchObject(["bottomTemperature", "surfaceTemperature", "station", "timestamp"].sort());
    const temperaturesWithStationParams = await fetchTemperatures([{ key: "Station", comparator: "=", value: "Fox Island" }]);
    ...
    const temperaturesWithTimeParams = await fetchTemperatures([{ key: "time", comparator: ">", value: dateMin }]);
    await expect(temperaturesWithTimeParams).toBeInstanceOf(Array);
    await expect(Math.min(...temperaturesWithTimeParams.map(({ timestamp }) => Number(timestamp)))).toBeGreaterThan(Number(dateMin));
});
...
test("fetchCoordinates", async () => {
    ...
    await expect(Object.keys(coordinates[0]).sort()).toMatchObject(["latitude", "longitude", "stationName"].sort())
});
